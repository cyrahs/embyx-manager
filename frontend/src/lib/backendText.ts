...
const EXACT_TEXT: Record<string, string> = {
  ...
  'Emby address and API key must be configured': '需要先在设置里配置 Emby 地址和 API Key',
  'playlist sync is not available in this deployment': '这个部署没有接入播放列表同步',
  'the source no longer publishes this list': '数据源已不再发布这张榜',
  'the API key was rejected': 'API Key 被拒绝',
