  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'interval_seconds', label: '运行间隔（秒）', kind: 'number' },
      {
        ...
        label: '分类与离线目录',
        kind: 'rss-categories',
        hint: '每行一个分类名，以及该分类的 115 离线目录（CloudDrive 云端路径，必填）。每次定时运行会按此顺序依次拉取所有分类，顺序也是优先级：同一番号被多个分类发现时归入排在前面的分类（如演员分类排在榜单前，榜单重列的作品仍落到演员目录）。每个离线目录都要在归档整理里配一条对应的来源子目录路由，完成的下载才会被归档；多个分类可以共用一个目录。删除分类前请确认它没有正在下载的任务——下载追踪只轮询这里列出的目录。',
      },
