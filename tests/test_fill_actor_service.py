...
from embyx_manager.fill_actor import (
    ApplyState,
    ExpiredPlanError,
    FillActorPaths,
    FillActorRuntime,
...
async def test_create_plan_preserves_partial_external_failures(paths: FillActorPaths) -> None:
    ...
    assert plan.actors[0].error_message == 'RuntimeError: upstream unavailable'
    assert plan.actors[1].error_code is None
    assert plan.actors[1].error_message is None
    videos = {video.video_id: video for video in plan.videos}
...
@pytest.mark.asyncio
async def test_actor_catalog_failure_is_logged_with_job_and_actor(
    paths: FillActorPaths,
    caplog: pytest.LogCaptureFixture,
) -> None:
    service, _ = make_service(
        paths,
        catalog={'broken': RuntimeError('upstream unavailable')},
        brands={},
    )

    with caplog.at_level(logging.ERROR, logger='embyx_manager.fill_actor.service'):
        await service.create_plan(['broken'], plan_id='job-42')

    record = next(record for record in caplog.records if 'actor catalog failed' in record.message)
    assert record.job_id == 'job-42'
    assert record.actor_id == 'broken'
...
@pytest.mark.asyncio
async def test_actor_catalog_failure_message_is_redacted_and_truncated(paths: FillActorPaths) -> None:
    leaky = RuntimeError(
        'GET https://catalog.invalid/actors?token=super-secret failed; '
        'Authorization: Bearer abcdef123456; cookie=session-value; ' + 'x' * 400
    )
    service, _ = make_service(paths, catalog={'broken': leaky}, brands={})

    plan = await service.create_plan(['broken'])

    message = plan.actors[0].error_message
    assert message is not None
    assert len(message) <= fill_actor_service_module.MAX_ERROR_MESSAGE_LENGTH
    assert message.startswith('RuntimeError: GET https://catalog.invalid/actors?[redacted]')
    for secret in ('super-secret', 'abcdef123456', 'session-value'):
...
@pytest.mark.skipif(
    sys.platform == 'darwin',
    # APFS inode reuse defeats external-replacement detection; also fails in
    # embyx-web on macOS. Linux CI exercises this path.
